...
public class JWTAuthenticationFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain) throws ServletException, IOException {
        ...
        if (uri.contains("/api/v1/auth/") || uri.contains("/api/users/all") || uri.startsWith("/api/users/search/") || uri.contains("/api/books/fav") || uri.contains("/api/books/read")  || uri.contains("/api/books/{userId}/read-books") || uri.contains("/api/books/{userId}/favorite-books") || uri.contains("/api/users/{userId}/theme") || uri.contains("/api/books/external/book/") || uri.contains("/api/follow/") || uri.contains("/api/follow/{id}/is-following") || uri.contains("/api/follow/{id}/following") || uri.contains("/api/follow/{id}/followers")  || uri.contains("/api/messages/") || uri.startsWith("/api/messages/conversation/{user1Id}/{user2Id}") || uri.startsWith("/api/messages/unread/{emisorId}/{receptorId}") || uri.startsWith("/api/messages/unread-counts-by-sender/{userId}") || uri.startsWith("/api/messages/chat.sendMessage") || uri.startsWith("/api/messages/chats/{userId}") || uri.startsWith("/ws") || uri.startsWith("/topic") || uri.startsWith("/queue") || uri.startsWith("/app")) {
